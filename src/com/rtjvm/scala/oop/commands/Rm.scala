...
class Rm(name: String) extends Command {
  ...
  def doRm(state: State, path: String): State = {
    ...
    def rmHelper(currDirectory: Directory, path: List[String]): Directory = {
      ...
      else if (path.tail.isEmpty) currDirectory.removeEntry(path.head)
      else {
        val nextDirectory = currDirectory.findEntry(path.head)
        if (!nextDirectory.isDirectory) currDirectory
        else {
          val newNextDirectory = rmHelper(nextDirectory.asDirectory, path.tail)
          if (currDirectory == newNextDirectory) currDirectory
